...
namespace Neodroid.Utilities.Messaging {
  ...
  public class MessageServer {
    ...
    public Messages.Reaction Receive(System.TimeSpan wait_time) {
      ...
      Messages.Reaction reaction = null;
      lock (this._thread_lock) {
        try {
          ...
          if (msg != null) { //&& msg.Length >= 4) {
            var flat_reaction = FBS.FReaction.GetRootAsFReaction(new ByteBuffer(msg));
            reaction = FBS.FbsReactionUtilities.unpack_reaction(flat_reaction);
          }
        } catch (System.Exception exception) {
      ...
      while (this._stop_thread == false) {
        lock (this._thread_lock) {
          if (!this._waiting_for_main_loop_to_send) {
            var reaction = this.Receive(System.TimeSpan.FromSeconds(this._wait_time_seconds));
            if (reaction != null) {
    ...
    public void SendStates(Messages.EnvironmentState[] environment_states) {
      lock (this._thread_lock) {
        ...
        this._byte_buffer = FBS.FbsStateUtilities.build_states(environment_states);
